...
object State:                                   //olika färger beroende på vad bokstaven har för status
...
def drawLetterBlock(pos: (Int, Int), letter: Char, state: java.awt.Color, size: Int): Unit =        //grafik för att rita ut en ruta med en bokstav i
    val x = pos(1) * (blockSize - size)
    val y = pos(0) * (blockSize - size)
    val offsetMap: Map[Char, Int] = Map('Q' -> 9, 'W' -> 5, 'E' -> 10, 'R' -> 9, 'T'-> 10, 'Y' -> 10, 'U' -> 10, 'I' -> 20, 'O' -> 8, 'P' -> 10, 'A' -> 10, 'S' -> 10, 'D' -> 10, 'F' -> 12, 'G' -> 7, 'H' -> 8, 'J' -> 13, 'K' -> 10, 'L' -> 12, 'Z' -> 10, 'X' -> 9, 'C' -> 7, 'V' -> 10, 'B' -> 10, 'N' -> 9, 'M' -> 6)
    ...
    if offsetMap.contains(letter) then
        window.drawText(letter.toString, (x + offsetMap(letter)) , y, java.awt.Color(90,90,90), blockSize - size - 10, 1, "SERIF")
    
    
def drawGame(): Unit =          //ritar ut tomma rutor när spelet startar
    for x <- 1 to 5 do 
        for y <- 1 to 6 do 
...
def drawKeys(): Unit =              //ritar ut "tangentbordet" när spelet startas
